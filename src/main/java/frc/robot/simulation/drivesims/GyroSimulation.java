...
public class GyroSimulation {
     ...
     *
     * <p>If you are using this class outside of {@link SimulatedArena}: make sure to call it 5 times in each robot
     * period (if using default timings), or refer to {@link SimulatedArena#overrideSimulationTimings(Time, int)}.
     *
    ...
    private Rotation2d getGyroDTheta(double actualAngularVelocityRadPerSec) {
        this.measuredAngularVelocityRadPerSec = RobotCommonMath.generateRandomNormal(
        ...
        final double
                AVERAGE_DRIFTING_1_PERIOD =
                        this.AVERAGE_DRIFTING_IN_30_SECS_MOTIONLESS_DEG
                                / 30
                                * SimulatedArena.getSimulationDt().in(Seconds),
                driftingInThisPeriod = RobotCommonMath.generateRandomNormal(0, AVERAGE_DRIFTING_1_PERIOD);
